// @flow
import {actionCreators as logActions} from './../../Store/UIStateActions';
...
const checkValidity = (dispatch, split_input, number_of_rovers) => {
    ...
    for (let i = 1; i < ((2 * number_of_rovers) + 1); i += 2) {
        ...
        // Regex to match any number, space, any number, space, then any valid heading
        if (!split_input[i].match(/^[0-9]+[ ][0-9]+[ ][NSEW]$/)) {
            return dispatch(logActions.logError(processError.invalid));
        }

        // Regex to match the allowed movement characters
...
export const actionCreators = {
    processInput: (text_input) => {
        ...
        // This is a thunk, it enables us to dispatch multiple actions
        return (dispatch) => {
            ...
            dispatch(actionCreators.setProcessedInput({
                rover_positions,
                grid_x,
                grid_y,
            }));
            ...
            dispatch(actionCreators.setProcessedOutput(output));
    ...
    setProcessedInput: (input) => {
        return {
            type: types.PROCESSED_INPUT,
            payload: input,
        };
    },
    setProcessedOutput: (output) => {
        return {
            type: types.PROCESSED_OUTPUT,
            payload: output,
        };
    },
    rawInput: (value) => {
        return {
            type: types.RAW_INPUT,
            payload: value,
        };
    },
    setResult: (payload) => {
